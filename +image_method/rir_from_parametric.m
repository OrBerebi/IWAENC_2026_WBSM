...
% end
%% delay to samples
delay = round(delay*fs); % renamed to avoid confusion with sec/samples

%% Calculate required padding
% We need extra samples at the end for the BPF and the anm2p filter transients.
% 1024 samples is usually enough for standard audio BPFs, or we can scale by fs.
tail_padding = max(1024, round(0.05 * fs)); % 50ms padding or 1024 samples

nref = size(doa,1);
q = 1:(N+1)^2;

% FIX: Add padding to the maximum delay so the IR isn't truncated
max_d = max(delay);
sz = [max_d + 1 + tail_padding, q(end)];
...
for i=1:buffer_size:nref
    ...
    % Use the local delay variable
    [xx, yy] = ndgrid(delay(I)+1, q);
    hnm = hnm + accumarray([xx(:) yy(:)], Yh_amp(:), sz); 
end
