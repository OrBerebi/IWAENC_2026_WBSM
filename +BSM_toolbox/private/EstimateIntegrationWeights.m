function [a] = EstimateIntegrationWeights(N, th, ph,type)
if nargin <4
    type = 'diagonal';
end
...
switch type
    case 'diagonal'
        coeff = [sqrt(4*pi);zeros((N+1)^2-1,1)];
        a = pinv(shinside(N,th,ph)) * coeff;
        
    case 'matrix'
        a = zeros((N+1)^2,numel(th));
        for nm = 1:(N+1)^2
            coeff = zeros((N+1)^2,1);
            coeff(nm) = 1;
        end
...
function Y = shinside(N,theta,phi)
