function fig=evaluation_gui(nbfile,expe,nbexpe,initPos, metricString)
...
fig=figure('Name','MUSHRAM - Evaluation phase','NumberTitle','off','MenuBar','none','Resize','off','Units','characters','Position',PositionFig);
movegui(fig,'center')

%%%displaying fixed items
%title
uicontrol(fig,'Style','Text','Units','characters','Position',[0 29 width ihight],'FontUnits','normalized','FontSize',FS+0.05,'String',['Experiment 1/' int2str(nbexpe)],'Tag','experiment');
uicontrol(fig,'Style','Text','Units','characters','Position',[0 27.5 width ihight],'FontUnits','normalized','FontSize',FS,'String',['Rate the similarity between the test signals and the reference with respect to ' metricString '.'],'Tag','experiment2');
...
results=uicontrol(fig,'Style','Pushbutton','Units','characters','FontUnits','normalized','FontSize',FS-0.15,'Tag','results','Callback','evaluation_callbacks(''results'',guidata(gcbo))');
if expe==nbexpe
    set(results,'Position',[width/2-8 1.5 25 2],'String','Save & exit (enter)');
else
    set(results,'Position',[width/2-8 1.5 25 2],'String','Save & proceed (enter)');
end
%play reference button
uicontrol(fig,'Style','Pushbutton','Units','characters','Position',[3.5 5.7 19 2],'FontUnits','normalized','FontSize',FS-0.15,'String','Play reference (1)','Callback','evaluation_callbacks(''play'',guidata(gcbo),0)','Tag','play0');
...
% Stop button
uicontrol(fig,'Style','Pushbutton','Units','characters','Position',[3.5 8 9 2],'FontUnits','normalized','FontSize',FS-0.15,'String','Stop (q)','Tag','Stop','Callback','evaluation_callbacks(''stop'',guidata(gcbo))');
...
for f=1:nbfile
    ...
    %slider
    uicontrol(fig,'Style','Slider','Units','characters','Position',[17.1+f*iwidth 11.24 2.8 13],'Min',0,'Max',100,'Value',0,'Callback',['evaluation_callbacks(''slider'',guidata(gcbo),' int2str(f) ')'],'Tag',['slider' int2str(f)]);
    %performance figures
    uicontrol(fig,'Style','Edit','Enable','inactive','Units','characters','Position',[14+f*iwidth 8.2 9 1.6],'FontUnits','normalized','FontSize',FS,'String',0,'Tag',['rating' int2str(f)]);
    %play buttons
    uicontrol(fig,'Style','Pushbutton','Units','characters','Position',[14+f*iwidth 5.7 9 2],'FontUnits','normalized','FontSize',FS-0.15,'String',['Play (' int2str(f+1) ')'],'Callback',['evaluation_callbacks(''play'',guidata(gcbo),' int2str(f) ')'],'Tag',['play' int2str(f)]);
end
set(fig,'WindowKeyPressFcn',@(hObject,eventdata)evaluation_callbacks('keyPressCallback',hObject,eventdata,guidata(hObject)));
set(fig,'CloseRequestFcn',@(hObject,eventdata)evaluation_callbacks('CloseFigure',hObject,eventdata,guidata(hObject)));
