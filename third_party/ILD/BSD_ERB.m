...
for k = 1:n
    ...
    X_L = repmat(abs(fft(C(:,k), Ncur)), [1, c]) .* (abs(fft(x_ref, Ncur)).^2);
    X_R = repmat(abs(fft(C(:,k), Ncur)), [1, c]) .* (abs(fft(x_test, Ncur)).^2);
    ...
    % get ERB error
    %X_out_ref(k,:) = 20*log10(abs(X_L) + eps);
    %X_out_test(k,:) = 20*log10(abs(X_R));
    X_out_ref(k,:) = X_L;
    X_out_test(k,:) = X_R;
